...
Before the first release, enable **immutable releases** in the repository
settings. GitHub locks the tag and assets only after the draft is published.
...
checksum verification succeed. It also refuses to update an existing release.
Repository immutability is an administrative prerequisite because the workflow
token cannot prove or enable that setting.
...
git push origin controlled-v0.155.1-rc.1
```
...
Each receipt records the repository, immutable tag, exact source commit and
...
digest and receipt commit against `SHA256SUMS` and the immutable release tag.
...
file in the archive. Publishing happens in the final job only. Failed
jobs leave no GitHub release.
